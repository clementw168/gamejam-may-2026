...
def init() -> None:
    ...
    try:
        _sounds["shoot"] = _load("shoot", lambda: _synth(
            duration=0.09, freq=900, freq_end=580, shape="sine",
            volume=0.22, attack=0.002, decay_rate=20))

        _sounds["dash"] = _load("dash", lambda: _synth(
            duration=0.13, freq=700, freq_end=220, shape="sine",
            volume=0.20, attack=0.003, decay_rate=14))

        _sounds["hit_wall"] = _load("hit_wall", lambda: _synth(
            duration=0.06, freq=300, shape="noise",
            volume=0.14, decay_rate=35))

        _sounds["hit_enemy"] = _load("hit_enemy", lambda: _synth(
            duration=0.10, freq=280, freq_end=180, shape="square",
            volume=0.22, attack=0.003, decay_rate=22))

        _sounds["enemy_death"] = _load("enemy_death", lambda: _synth(
            duration=0.30, freq=360, freq_end=90, shape="sine",
            volume=0.24, attack=0.008, decay_rate=7,
            vibrato_hz=18, vibrato_rate=14))

        _sounds["player_hurt"] = _load("player_hurt", lambda: _synth(
            duration=0.22, freq=220, freq_end=110, shape="square",
            volume=0.30, attack=0.005, decay_rate=9))

        _sounds["coin"] = _load("coin", lambda: _synth(
            duration=0.15, freq=1500, freq_end=1900, shape="sine",
            volume=0.18, attack=0.003, decay_rate=22))

        _sounds["spore_shoot"] = _load("spore_shoot", lambda: _synth(
            duration=0.14, freq=260, freq_end=160, shape="sine",
            volume=0.16, attack=0.010, decay_rate=15))

        _sounds["room_clear"] = _load("room_clear", lambda: _synth(
            duration=0.50, freq=523, freq_end=784, shape="sine",
            volume=0.22, attack=0.015, decay_rate=4))

        _sounds["wolf_lunge"] = _load("wolf_lunge", lambda: _synth(
            duration=0.12, freq=500, freq_end=150, shape="square",
