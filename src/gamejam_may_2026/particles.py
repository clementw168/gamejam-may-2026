...
class ParticleSystem:
    ...
    def emit(
        ...
        for _ in range(count):
            ...
            self._particles.append(
                Particle(x, y, math.cos(a) * spd, math.sin(a) * spd, col, lt, sz, gravity)
            )
    ...
    def emit_hit(self, x: float, y: float, angle_deg: float) -> None:
        ...
        self.emit(x, y, 7,
                  [C.C_SPARK_A, C.C_SPARK_B, (255, 255, 180)],
                  190, angle_deg + 180, 65, 0.38, size=2.5, gravity=220)

    def emit_enemy_hit(self, x: float, y: float) -> None:
        """Blood/leaf particles when an arrow hits an enemy."""
        self.emit(x, y, 9,
                  [C.C_BLOOD_A, C.C_BLOOD_B, (255, 100, 100)],
                  130, 0, 180, 0.45, size=3.2, gravity=100)
    ...
    def emit_death(
        ...
        self.emit(x, y, 14,
                  [color_a, color_b, (255, 255, 255)],
                  150, 0, 180, 0.65, size=4.0, gravity=130)
        # Leaf scatter
        self.emit(x, y, 6,
                  [C.C_LEAF_A, C.C_LEAF_B],
                  80, 270, 80, 0.9, size=2.5, gravity=180)

    def emit_player_hurt(self, x: float, y: float) -> None:
        self.emit(x, y, 10,
                  [(255, 150, 150), (255, 80, 80)],
                  160, 270, 180, 0.55, size=3.5, gravity=120)

    def emit_dash_trail(self, x: float, y: float) -> None:
        self.emit(x, y, 4,
                  [C.C_DASH_TRAIL, (180, 230, 255)],
                  40, random.uniform(0, 360), 180, 0.22, size=3.5)

    def emit_coin_pickup(self, x: float, y: float) -> None:
        self.emit(x, y, 5,
                  [C.C_COIN, (255, 240, 100)],
                  100, 270, 70, 0.42, size=2.5, gravity=-60)

    def emit_wolf_lunge(self, x: float, y: float, angle_deg: float) -> None:
        """Speed-blur when wolf lunges."""
        self.emit(x, y, 8,
                  [C.C_WOLF, (200, 185, 168), (255, 255, 255)],
                  70, angle_deg + 180, 40, 0.28, size=3.0)

    def emit_spore_death(self, x: float, y: float) -> None:
        """Plant explodes in a shower of spores."""
        self.emit(x, y, 16,
                  [(75, 185, 55), (55, 140, 38), (130, 210, 90)],
                  135, 0, 180, 0.75, size=4.5, gravity=100)
        self.emit(x, y, 8,
                  [(200, 240, 100), (160, 220, 60)],
                  60, 270, 80, 1.0, size=2.5, gravity=200)

    def emit_room_clear(self, cx: float, cy: float) -> None:
        """Celebratory burst when the last enemy dies."""
        self.emit(cx, cy, 25,
                  [C.C_COIN, (200, 255, 150), (255, 255, 255)],
                  220, 0, 180, 0.9, size=4.5, gravity=80)

    def emit_shaman_phase2(self, x: float, y: float) -> None:
        """Phase-2 burst when Goblin Shaman crosses 50 % HP."""
        self.emit(x, y, 22,
                  [(205, 100, 255), (165, 50, 200), (255, 255, 255)],
                  200, 0, 180, 0.75, size=4.5)

    def emit_tree_phase2(self, x: float, y: float) -> None:
        """Phase-2 burst when Ancient Tree crosses 50 % HP."""
        self.emit(x, y, 24,
                  [(88, 55, 20), (130, 195, 55), (200, 240, 100)],
                  180, 0, 180, 0.85, size=5.0)
        self.emit(x, y, 10,
                  [(255, 255, 255), (200, 240, 100)],
                  80, 270, 80, 1.1, size=2.5, gravity=200)

    def emit_shaman_death(self, x: float, y: float) -> None:
        self.emit(x, y, 20,
                  [(165, 90, 210), (205, 100, 255), (255, 220, 255)],
                  170, 0, 180, 0.80, size=5.0)
        self.emit(x, y, 8,
                  [(255, 200, 255), (200, 100, 255)],
                  90, 270, 70, 1.0, size=3.0, gravity=150)

    def emit_tree_death(self, x: float, y: float) -> None:
        self.emit(x, y, 28,
                  [(75, 185, 55), (42, 88, 28), (130, 210, 90)],
                  200, 0, 180, 0.90, size=6.0, gravity=80)
        self.emit(x, y, 12,
                  [(88, 55, 20), (130, 195, 55)],
                  80, 270, 80, 1.2, size=3.5, gravity=200)
        self.emit(x, y, 6,
                  [(255, 255, 255), (200, 240, 100)],
                  120, 0, 180, 0.6, size=2.0)
