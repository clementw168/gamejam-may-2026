...
class Camera:
    ...
    def apply_pos(self, x: float, y: float) -> tuple[float, float]:
        ...
        return (x - self.offset.x + self._sx, y - self.offset.y + self._sy)

    def apply_vec(self, v: pygame.Vector2) -> pygame.Vector2:
        return pygame.Vector2(v.x - self.offset.x + self._sx, v.y - self.offset.y + self._sy)

    def apply_rect(self, rect: pygame.Rect) -> pygame.Rect:
        return rect.move(int(-self.offset.x + self._sx), int(-self.offset.y + self._sy))

    def screen_to_world(self, sx: float, sy: float) -> tuple[float, float]:
        return (sx + self.offset.x - self._sx, sy + self.offset.y - self._sy)
