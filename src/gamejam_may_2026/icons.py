...
def _star_pts(cx: float, cy: float, r_out: float, r_in: float,
              n: int = 5, offset: float = -math.pi / 2) -> list[tuple[int, int]]:
    pts = []
    for i in range(n * 2):
...
def _arrow_right(cx: float, cy: float, r: float) -> list[tuple[int, int]]:
    ...
    hd = max(1, r * 2 / 3)
    return [
        _p(cx, cy, -r,     -hw), _p(cx, cy,  r / 3, -hw),
        _p(cx, cy,  r / 3, -hd), _p(cx, cy,  r,      0),
        _p(cx, cy,  r / 3,  hd), _p(cx, cy,  r / 3,  hw),
        _p(cx, cy, -r,      hw),
    ]
...
def _arrow_up(cx: float, cy: float, r: float) -> list[tuple[int, int]]:
    ...
    hd = max(1, r * 2 / 3)
    return [
        _p(cx, cy, -hw,  r),    _p(cx, cy, -hw, -r / 3),
        _p(cx, cy, -hd, -r / 3), _p(cx, cy,  0,  -r),
        _p(cx, cy,  hd, -r / 3), _p(cx, cy,  hw, -r / 3),
        _p(cx, cy,  hw,  r),
    ]
...
def _swift_boots(s, cx, cy, r, c):
    """Lightning bolt."""
    pts = [
        _p(cx, cy, -r * 0.15, -r),    _p(cx, cy,  r * 0.40, -r),
        _p(cx, cy,  0,          0.1 * r), _p(cx, cy,  r * 0.55,  0.1 * r),
        _p(cx, cy, -r * 0.40,   r),    _p(cx, cy,  r * 0.05,  0),
        _p(cx, cy, -r * 0.50,   0),
    ]
    pygame.draw.polygon(s, c, pts)
...
def _piercing_shot(s, cx, cy, r, c):
    ...
    for dy in (-r * 0.55, r * 0.55):
        pygame.draw.line(s, _bright(c, 1.3),
                         _p(cx, cy, -r * 0.8, dy), _p(cx, cy, -r * 0.2, dy), lw)
...
def _double_shot(s, cx, cy, r, c):
    ...
    for dy in (-gap, gap):
        ...
        tip = int(cx + r * 0.85)
        pts = [
            (sl, int(cy + dy - hw)), (sr, int(cy + dy - hw)),
            (sr, int(cy + dy - hd)), (tip, int(cy + dy)),
            (sr, int(cy + dy + hd)), (sr, int(cy + dy + hw)),
            (sl, int(cy + dy + hw)),
        ]
        pygame.draw.polygon(s, c, pts)
...
def _dash_master(s, cx, cy, r, c):
    """Hollow diamond."""
    pts = [(int(cx), int(cy - r)), (int(cx + r), int(cy)),
           (int(cx), int(cy + r)), (int(cx - r), int(cy))]
    pygame.draw.polygon(s, c, pts)
    ri = max(1, int(r * 0.55))
    inner = [(int(cx), int(cy - ri)), (int(cx + ri), int(cy)),
             (int(cx), int(cy + ri)), (int(cx - ri), int(cy))]
...
def _iron_hide(s, cx, cy, r, c):
    ...
    pygame.draw.line(s, _dim(c, 0.4),
                     (int(cx - r * 0.5), int(cy)), (int(cx + r * 0.5), int(cy)), bw)
    pygame.draw.line(s, _dim(c, 0.4),
                     (int(cx), int(cy - r * 0.5)), (int(cx), int(cy + r * 0.5)), bw)
...
def _swift_arrow(s, cx, cy, r, c):
    ...
    for dy in (-r * 0.55, r * 0.55):
        pygame.draw.line(s, _bright(c, 1.3),
                         _p(cx, cy, -r * 0.9, dy), _p(cx, cy, -r * 0.4, dy), lw)
...
def _coin_magnet(s, cx, cy, r, c):
    ...
    outer = [_p(cx, cy, r * math.cos(math.pi + math.pi * i / n),
                        r * math.sin(math.pi + math.pi * i / n)) for i in range(n + 1)]
    # Inner arch (right → top → left)
    inner = [_p(cx, cy, ri * math.cos(2 * math.pi - math.pi * i / n),
                        ri * math.sin(2 * math.pi - math.pi * i / n)) for i in range(n + 1)]
...
def _wardens_mark(s, cx, cy, r, c):
    ...
    pygame.draw.polygon(s, c, pts)
    lw = max(1, r // 3)
    pygame.draw.line(s, _dim(c),
                     (int(cx), int(cy - r * 0.5)), (int(cx), int(cy + r * 0.5)), lw)
    pygame.draw.line(s, _dim(c),
                     (int(cx - r * 0.4), int(cy)), (int(cx + r * 0.4), int(cy)), lw)


def _echoing_shot(s, cx, cy, r, c):
    """Arc + arrowhead (echo arrow)."""
    lw = max(1, r // 3)
    arc_rect = (int(cx - r * 0.7), int(cy - r * 0.8),
                int(r * 1.4), int(r * 1.4))
...
def _iron_lungs(s, cx, cy, r, c):
    ...
    for yo, xo in offsets:
        pygame.draw.line(s, c,
                         (int(cx - r + xo), int(cy + yo)),
                         (int(cx + r - xo), int(cy + yo)), lw)
...
def _bone_buckler(s, cx, cy, r, c):
    ...
    for (ax, ay), (bx, by) in [
        ...
        pygame.draw.line(s, c,
                         (int(cx + ax * 0.80), int(cy + ay * 0.80)),
                         (int(cx + bx * 0.80), int(cy + by * 0.80)), lw)
...
def _coin_fed_heart(s, cx, cy, r, c):
    ...
    pygame.draw.circle(s, _dim(c, 0.4), (int(cx + r * 0.6), int(cy - r * 0.3)), cr,
                       max(1, cr // 3))
...
def _shrapnel_tips(s, cx, cy, r, c):
    ...
    for i in range(8):
        ...
        pygame.draw.line(s, c,
                         ...
                         (int(cx + ro * math.cos(a)), int(cy + ro * math.sin(a))), lw)
    pygame.draw.circle(s, c, (int(cx), int(cy)), max(2, r // 4))
...
def _leech_stone(s, cx, cy, r, c):
    ...
    # Shine dot
    pygame.draw.circle(s, _bright(c, 1.4),
                       (int(cx - r * 0.18), int(cy - r * 0.38)), max(1, r // 7))


def _overcharged_quiver(s, cx, cy, r, c):
    """Bold lightning bolt (larger/fatter than swift_boots)."""
    pts = [
        _p(cx, cy, -r * 0.10, -r),    _p(cx, cy,  r * 0.50, -r),
        _p(cx, cy,  r * 0.05,  0),    _p(cx, cy,  r * 0.70,  0),
        _p(cx, cy, -r * 0.50,  r),    _p(cx, cy,  r * 0.0,   0),
        _p(cx, cy, -r * 0.60,  0),
    ]
    pygame.draw.polygon(s, c, pts)
...
def _echo_chamber(s, cx, cy, r, c):
    ...
    for ar in (r * 0.5, r * 0.72, r * 0.95):
        pygame.draw.arc(s, c,
                        (int(cx + ox + r * 0.05), int(cy - ar),
                         int(ar * 2), int(ar * 2)),
                        -math.pi * 0.45, math.pi * 0.45, lw)
...
def _temporal_blur(s, cx, cy, r, c):
    ...
    for ar, start, span in [
        ...
        pygame.draw.arc(s, c,
                        (int(cx - ar), int(cy - ar), int(ar * 2), int(ar * 2)),
                        start, start + span, lw)
...
def _runic_arrows(s, cx, cy, r, c):
    ...
    for dx in (-r * 0.55, -r * 0.2):
        pygame.draw.line(s, _dim(c),
                         (int(cx + dx), int(cy - r * 0.42)),
                         (int(cx + dx), int(cy + r * 0.42)), lw)
...
def _bloodlust(s, cx, cy, r, c):
    ...
    # Inner flicker
    pygame.draw.circle(s, _bright(c, 1.4),
                       (int(cx), int(cy + r * 0.18)), max(1, cr // 2))
...
def _curse_of_greed(s, cx, cy, r, c):
    ...
    tw = max(1, r // 4)
    for i in range(-1, 2):
        pygame.draw.rect(s, bg, (int(cx + i * r * 0.36 - tw // 2),
                                 int(cy + r * 0.38), tw, int(r * 0.38)))
...
def _petrified_heart(s, cx, cy, r, c):
    ...
    pygame.draw.polygon(s, c, pts)
    lw = max(1, r // 5)
    pygame.draw.line(s, _dim(c, 0.40),
                     (int(cx - r * 0.3), int(cy - r * 0.25)),
                     (int(cx + r * 0.1), int(cy + r * 0.40)), lw)
...
def _void_core(s, cx, cy, r, c):
    ...
    pygame.draw.circle(s, _dim(c, 0.65), (int(cx), int(cy)),
                       int(r * 0.52), max(1, lw // 2))
...
def draw(surf: pygame.Surface, cx: int, cy: int,
         size: int, icon_id: str, color: tuple) -> None:
